...
def create_mask(width, height, mask_width=None, mask_height=None, x=None, y=None, d_x=None, d_y=None):
    if not mask_width:
        mask_width = random.randint(width // 8, width // 8 * 7)
        # mask_width = width // 2
    if not mask_height:
        mask_height = random.randint(height // 8, min(height // 8 * 7, height * width // 4 // mask_width))
    ...
    if not d_y:
        d_y = 0
    mask_x = x if x is not None else random.randint(d_x, width - mask_width - d_x)
    mask_y = y if y is not None else random.randint(d_y, height - mask_height - d_y)
