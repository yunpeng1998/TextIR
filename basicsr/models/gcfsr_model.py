...
class GCFSR_Model(BaseModel):
    ...
    def init_training_settings(self):
        ...
        self.eval_text = ['face with closed eyes', 
                            ...
                            'face of a person with one large nostril and one small nostril',
                            'wrinkled person',
                            'old man',
                            'face of a vampire',
                            'tight-lipped man',
    ...
    def feed_data(self, data):
        ...
        self.train_fea = self.img_fea.clone().detach()
        if 'tao' in self.opt['train']:
            self.train_fea = self.train_fea + \
                                torch.normal(mean=0, 
                                            std=self.opt['train']['tao'], 
                                            size=self.train_fea.size(), 
                                            device=self.train_fea.device)
        if self.opt['train']['norm']:
            self.img_fea = self.img_fea / self.img_fea.norm(dim=-1, keepdim=True)
            self.train_fea = self.train_fea / self.train_fea.norm(dim=-1, keepdim=True)
    ...
    def optimize_parameters(self, current_iter):
        ...
        fake_img, _ = self.net_g(self.lq, self.train_fea)
