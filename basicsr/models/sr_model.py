...
class SuperResolutionModel(BaseModel):
    ...
    def __init__(self, opt):
        super(SuperResolutionModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        self.eval_text = ['face with closed eyes', 
                            'face with blue eyes',
                            'face with glasses',
                            'face with makeup',
                            'sad face',
                            ...
                            'face of a vampire',
                            'tight-lipped man',
                            'person with an eagle nose',
                            'face with yellow skin',
                            'face with black skin',
                            'person with a painted face']
    ...
    def optimize_parameters(self, current_iter):
        ...
        fake_img, _ = self.net_g(self.lq, self.img_fea, self.in_size)
        # fake_img = self.real_img * (1 - self.mask) + fake_img * self.mask
        ...
        if self.cri_pix:
            ...
            for i in range(fake_img.size(0)):
                scale = self.in_size[i][0] * 64
                scale = scale.item()
                temp = F.interpolate(fake_img[i:i+1, :, :, :], scale_factor=1/scale, mode='bicubic', align_corners=True)
                fake_img_batch.append(F.interpolate(temp, scale_factor=scale, mode='bicubic', align_corners=True))
            fake_img_batch = torch.cat(fake_img_batch, dim=0)
            l_g_pix = self.cri_pix(fake_img_batch, self.lq)
            """
    ...
    def test(self, cond=None):
        if cond is not None:
            with torch.no_grad():
                self.net_g_ema.eval()
                self.output, _ = self.net_g_ema(self.lq, cond, self.in_size)
                # self.output = self.real_img * (1 - self.mask) + self.output * self.mask
        else:
            with torch.no_grad():
                self.net_g_ema.eval()
                self.output, _ = self.net_g_ema(self.lq, self.img_fea, self.in_size)
    ...
    def eval_net(self, current_iter):
        ...
        vutils.save_image(torch.cat([self.real_img[:4], self.lq[:4, :3, :, :], self.output[:4]], dim=0), save_path, nrow=4)
        ...
        save_path = os.path.join(self.opt['path']['visualization'], save_name)
        vutils.save_image(torch.cat([self.real_img[:4], self.lq[:4, :3, :, :], self.output[:4], img_shuffle[:4]], dim=0), save_path, nrow=4)
